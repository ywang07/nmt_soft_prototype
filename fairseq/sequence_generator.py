...
class SequenceGenerator(object):
    ...
    def _generate(self, input, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        for model in self.models:
            ...
            if isinstance(model.decoder, FairseqIncrementalDecoder):
                incremental_states[model] = ({}, {}) if getattr(
                    model.decoder, "share_encoder_params", False) else {}
            else:
                incremental_states[model] = None

            # compute the encoder output for each beam
            encoder_out = model.encoder(**self.prepare_input(input, beam_size))
    ...
    @staticmethod
    def prepare_input(input, beam_size):
        encoder_input = dict()
        encoder_input['src_tokens'] = input['src_tokens'].repeat(
            1, beam_size).view(-1, input['src_tokens'].size(1))
        encoder_input['src_lengths'] = input['src_lengths'].expand(
            beam_size, input['src_lengths'].numel()).t().contiguous().view(-1)

        src_tokens_2 = input.get('src_tokens_2')
        src_lengths_2 = input.get('src_lengths_2')
        src_scores_2 = input.get('src_scores_2')
        if src_scores_2 is not None:
            encoder_input['src_scores_2'] = src_scores_2.repeat(
                1, beam_size, 1).view(-1, src_scores_2.size(1), src_scores_2.size(2))
            encoder_input['src_tokens_2'] = src_tokens_2.repeat(
                1, beam_size, 1).view(-1, src_tokens_2.size(1), src_tokens_2.size(2))
            encoder_input['src_lengths_2'] = src_lengths_2.expand(
                beam_size, src_lengths_2.numel()).t().contiguous().view(-1) \
                if src_lengths_2 is not None else None
        elif src_tokens_2 is not None:
            encoder_input['src_tokens_2'] = src_tokens_2.repeat(
                1, beam_size).view(-1, src_tokens_2.size(1))
            encoder_input['src_lengths_2'] = src_lengths_2.expand(
                beam_size, src_lengths_2.numel()).t().contiguous().view(-1) \
                if src_lengths_2 is not None else None
