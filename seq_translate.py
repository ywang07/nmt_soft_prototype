...
def obtain_sys_argv():
    def if_illegal_args(str_check):
        illegal_args_names = ['--ckpt', '--initial', '--fairseq-dir']
        return any([x in str_check for x in illegal_args_names])
    sys_args = ' '.join(
        [x for (idx, x) in enumerate(sys.argv[1:])
         if not if_illegal_args(x) and not if_illegal_args(sys.argv[idx])])
    return sys_args
...
def main(args):
    ...
    for x in range(start_idx, len(files)):
        ...
        print('python {}/generate.py {} --path {}'.format(args.fairseq_dir, obtain_sys_argv(), ckpt_file))
        pl_process = subprocess.Popen(
            'python {}/generate.py {} --path {}'.format(args.fairseq_dir, obtain_sys_argv(), ckpt_file),
            shell=True,
